 ...
 * https://github.com/v-v/karkkainen-sanders/blob/master/code/java/Bioinformatika/src/SuffixArray.java
 */

public class FastSuffixArray {
    public static int[] create(char[] genome) {

        // convert string to array of integers
        Set<Character> alphabet = new HashSet<>();
        for (Character c : genome) alphabet.add(c);

        System.out.println("Alphabet: " + alphabet);

        List<Character> alphabetList = new ArrayList<>(List.copyOf(alphabet));
        alphabetList.sort(Comparator.naturalOrder());

        Map<Character, Integer> charRank = new HashMap<>();
        for (int i = 0; i < alphabetList.size(); i++) {
            charRank.put(alphabetList.get(i), i);
        }

        int[] sequence = new int[genome.length];
        for (int i = 0; i < genome.length; i++) {
            if(!charRank.containsKey(genome[i]))
                throw new RuntimeException("Invalid character in sequence: " + genome[i]);
            sequence[i] = charRank.get(genome[i]) + 1;
        }
        return constructSuffixArray(sequence, alphabet.size());
    }
